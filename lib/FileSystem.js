...
export class FileSystem {
  ...
  async getLocalFilePathFromUrl(url, permanent) {
    ...
    let fileExists = await RNFetchBlob.fs.exists(path);
    if (fileExists) {
      filePath = path;
    } else {
      this.fetchFile(url, permanent, null, true);
      return null;
    }
    return filePath;
  }
  ...
  async fetchFile(url, permanent = false, fileName = null, clobber = false) {
    ...
    let directory = this.baseFilePath + (permanent ? 'permanent' : 'cache') + '/';
    let path = directory + fileName;
    // Logic here prunes cache directory on "cache" writes to ensure cache doesn't get too large.
    if (!permanent) {
      await this.pruneCache();
    }
    let result = await RNFetchBlob.fetch('GET', url);
    if (result && result.respInfo) {
      let { status } = result.respInfo;
      if (parseInt(status) == 200) {
        const base64Content = await result.base64();
        const isDirExists = await RNFetchBlob.fs.exists(directory);
        if (!isDirExists) {
          await RNFetchBlob.fs.mkdir(directory);
        }
        await RNFetchBlob.fs.createFile(path, base64Content, "base64");
      }
    }
